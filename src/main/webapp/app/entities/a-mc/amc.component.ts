...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/amc'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: AMC) {
        return item.id;
    }
    registerChangeInAMCS() {
        this.eventSubscriber = this.eventManager.subscribe('aMCListModification', (response) => this.loadAll());
    }
