...
currentAccount: any;
    ...
    loadAll() {
        this.fixedDepositService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/fixed-deposit'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: FixedDeposit) {
        return item.id;
    }
    registerChangeInFixedDeposits() {
        this.eventSubscriber = this.eventManager.subscribe('fixedDepositListModification', (response) => this.loadAll());
    }
