...
@Component({
    selector: 'jhi-mf-portfolio',
    templateUrl: './mf-portfolio.component.html'
})
export class MFPortfolioComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/mf-portfolio'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: MFPortfolio) {
        return item.id;
    }
    registerChangeInMFPortfolios() {
        this.eventSubscriber = this.eventManager.subscribe('mFPortfolioListModification', (response) => this.loadAll());
    }
