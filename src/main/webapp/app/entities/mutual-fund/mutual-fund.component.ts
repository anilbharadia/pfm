...
export class MutualFundComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/mutual-fund'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: MutualFund) {
        return item.id;
    }
    registerChangeInMutualFunds() {
        this.eventSubscriber = this.eventManager.subscribe('mutualFundListModification', (response) => this.loadAll());
    }
